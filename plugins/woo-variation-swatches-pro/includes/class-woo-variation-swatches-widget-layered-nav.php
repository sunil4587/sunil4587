	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( wvs_is_color_attribute( $attribute ) ) {
				...
				if ( $count > 0 || $option_is_set ) {
					$link      = apply_filters( 'woocommerce_layered_nav_link', $link, $term, $taxonomy );
					$term_html = '<a rel="nofollow" href="' . esc_url( $link ) . '"><div data-wvstooltip="' . esc_attr( $term->name ) . '" class="wvs-widget-item-wrapper"><span class="item" style="background-color: ' . esc_attr( $color ) . '"></span><span class="text">' . esc_html( $term->name ) . '</span></div></a>';
				} else {
					$link      = false;
					$term_html = '<div data-wvstooltip="' . esc_attr( $term->name ) . '" class="wvs-widget-item-wrapper"><span class="item" style="background-color: ' . esc_attr( $color ) . '"></span><span class="text">' . esc_html( $term->name ) . '</span></div>';
				}
			} else {
