...
class XT_Woo_Points_Rewards_Manager {
	...
	public static function event_type_description( $event_type, $event = null ) {
		...
		switch ( $event_type ) {
			case 'admin-reset': 	 $event_description = sprintf( __( '%s reset by "admin"',          	  'xt-woo-points-rewards' ), $points_label ); break;
			case 'admin-adjustment': $event_description = sprintf( __( '%s adjusted by "admin"',          'xt-woo-points-rewards' ), $points_label ); break;
			case 'order-placed':     $event_description = sprintf( __( '%s earned for purchase',          'xt-woo-points-rewards' ), $points_label ); break;
			case 'order-cancelled':  $event_description = sprintf( __( '%s adjusted for cancelled order', 'xt-woo-points-rewards' ), $points_label ); break;
			case 'order-refunded':   $event_description = sprintf( __( '%s adjusted for an order refund', 'xt-woo-points-rewards' ), $points_label ); break;
			case 'order-redeem':     $event_description = sprintf( __( '%s redeemed towards purchase',    'xt-woo-points-rewards' ), $points_label ); break;
			case 'expire':           $event_description = sprintf( __( '%s expired',                      'xt-woo-points-rewards' ), $points_label ); break;
		}

		// allow other plugins to define their own event types/descriptions
		return apply_filters( xt_woo_points_rewards()->plugin_prefix('event_description'), $event_description, $event_type, $event );
	}
	...
	public static function set_points_balance( $user_id, $points_balance, $event_type ) {
		...
		$points_balance = apply_filters( 'xt_woopr_set_points_balance', $points_balance, $user_id, $event_type );

		if(!is_numeric($points_balance)) {
            $points_balance = 0;
        }
		...
		update_user_meta( $user_id, 'xt_woopr_balance', $points_balance );
		...
		if ( $points_change ) {
			...
			// log the event
			XT_Woo_Points_Rewards_Points_Log::add_log_entry( $args );
	...
	public static function increase_points( $user_id, $points, $event_type, $data = null, $order_id = null ) {
		...
		$points = apply_filters( 'xt_woopr_increase_points', $points, $user_id, $event_type, $data, $order_id );
		...
		$points_balance = (int) get_user_meta( $user_id, 'xt_woopr_balance' );
		update_user_meta( $user_id, 'xt_woopr_balance', $points_balance + $points );

		do_action( 'xt_woopr_after_increase_points', $user_id, $points, $event_type, $data, $order_id );
	...
	public static function decrease_points( $user_id, $points, $event_type, $data = null, $order_id = null ) {
		...
		$points = apply_filters( 'xt_woopr_decrease_points', $points, $user_id, $event_type, $data, $order_id );
		...
		do_action( 'xt_woopr_after_reduce_points', $user_id, $points_balance );
	...
	public static function delete_user_points( $user_id ) {
		...
		$wpdb->delete( xt_woo_points_rewards()->user_points_db_tablename, array( 'user_id' => $user_id ) );
	}
	 ...
	 */
	public static function get_users_points( $user_id = null) {

        global $wpdb;

        $user_id = $user_id ? $user_id : get_current_user_id();

        $cache_key = xt_woo_points_rewards()->plugin_short_prefix('get_users_points'.$user_id);
        $points_balance = wp_cache_get( $cache_key );

        if( false === $points_balance) {
            ...
            $points_balance = is_numeric($points_balance) ? $points_balance : null;
            wp_cache_set( $cache_key, $points_balance );
        }

		return apply_filters( 'xt_woopr_user_points_balance', $points_balance, $user_id );
	}
	...
	public static function calculate_points_value( $amount ) {
		...
		return number_format( $amount * ( $ratio->monetary_value / $ratio->points ), 2, '.', '' );
	}
