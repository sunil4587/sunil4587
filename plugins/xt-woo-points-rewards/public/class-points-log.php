...
class XT_Woo_Points_Rewards_Points_Log {
	...
	public static function add_log_entry( $args ) {
		...
		// create the record
		return $wpdb->insert( xt_woo_points_rewards()->user_points_log_db_tablename,
			$data,
			$format
		);
	}
	...
	public static function get_points_log_entries( $args ) {
		...
		if ( ! empty( $args['user'] ) ) {
			$args['where'][] = $wpdb->prepare( xt_woo_points_rewards()->user_points_log_db_tablename.".user_id = %s", $args['user'] );
		}

		// special handling for searching by event type
		if ( ! empty( $args['event_type'] ) ) {
			$args['where'][] = $wpdb->prepare( xt_woo_points_rewards()->user_points_log_db_tablename.".type = %s", $args['event_type'] );
		}
	...
	public static function get_event_types() {
		...
		$query = "SELECT type, COUNT(*) as count FROM ".xt_woo_points_rewards()->user_points_log_db_tablename." GROUP BY type ORDER BY type";
	...
	public static function query( $args ) {
		...
		// returned fields
		$fields = xt_woo_points_rewards()->user_points_log_db_tablename.".*";
		...
		// build the query
		$query = "SELECT $found_rows $distinct $fields FROM ".xt_woo_points_rewards()->user_points_log_db_tablename." $join WHERE 1=1 $where $groupby $orderby $limits";
